...
def swap_nibbles(data: Iterable[int]) -> bytes:
    """Exchange the high and low nibble of every byte in *data*.

    The SidStation stores every numeric patch *parameter* byte with its two
    nibbles swapped relative to the value the synth actually works with -- so
    a stored ``0x21`` is transpose ``+18`` semitones (0x12), a stored ``0xA0`` is
    a release of ``0x0A`` = 10, an LFO ctrl_type's *type* lives in the high
    nibble of the stored byte, and so on.  (The 10-byte ASCII name is *not*
    nibble-encoded and is exempt.)  This is cross-checked against the whole
    factory bank.  :meth:`Patch.from_sysex` applies this once to the decoded
    parameter region so the in-memory :attr:`Patch.data` holds the synth's true
    values; :meth:`Patch.to_sysex` applies it again (it is its own inverse) on
    the way out, so files still round-trip byte-for-byte.
    """
