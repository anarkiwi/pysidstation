...
- **Oscillators** — `patch.oscillators[0..2]` with `waveform`, `ring_mod`,
  `sync`, `gate`, `track`, `arp_speed`, `transpose`, `detune`,
  `pitchbend_range`, `attack`, `decay`, `sustain`, `release`, `delay`,
  `pwm_start`, `pwm_add`, `pwm_lfo`, `pwm_lfo_depth`, `portamento`,
  `vibrato_lfo`, `vibrato_depth`, `vibrato_wheel_depth`, `table_speed`
...
bank. Two corrections matter in practice:
...
- **The oscillator waveform is stored in the low nibble**, not the high nibble
  the manual diagrams suggest.

Both are explained in [`docs/FORMAT.md`](docs/FORMAT.md).
