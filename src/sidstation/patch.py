...
class Oscillator:
    ...
    Raw byte values are exposed directly (see the manual for the documented
    ranges; the hardware does not always clamp to them).  ``waveform``,
    ``ring_mod`` and ``sync`` decode the OSC_WAVE byte, and ``sync_pwm`` /
    ``gate`` decode the OSC_FLAGS byte.
    """
    ...
    wave = Byte(15)
    waveform = Bits(15, 0, 4)  # low nibble: 1=Tri 2=Saw 4=Pulse 5=Mixed 8=Noise
    sync = Flag(15, 5)
    ring_mod = Flag(15, 6)
    portamento = Byte(16)
    vibrato_lfo = Byte(17)
...
class TableStep:
    ...
    def waveform(self) -> int:
        """Waveform code (low nibble of DATA1)."""
        return self.data1 & 0x0F
    ...
    def note_value(self) -> int:
        ...
        if self.data2 < 0x80:
            return self.data2
        if self.data2 < 0xC0:
            return self.data2 - 0x80
        return self.data2 - 0xC0
...
class Patch:
    ...
    def from_sysex(cls, message: bytes, prefix: bytes | None = None) -> Patch:
        ...
        decoded = decode_nibbles(body[NAME_LEN:])
    ...
    def to_sysex(self, prefix: bytes = None) -> bytes:
        ...
        out += encode_nibbles(self._data[NAME_LEN:])
